...
export function SplitFlapMuteToggle({ className = "" }: { className?: string }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-[#a1a1aa] hover:text-[#18181b] transition-colors duration-200 ${className}`}
...
const CHARSET = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789.".split("") // Added dot for parallax.ai
  ...
  return (
    ...
    >
      {chars.map((char, index) => (
        <SplitFlapChar
          key={index}
          char={char.toUpperCase()} // we uppercase but it will only match uppercase things or .
          index={index}
          ...
          actualChar={char} // we pass original char to show it with proper case if specified.
        />
      ))}
    </div>
  )
...
function SplitFlapChar({ char, actualChar, index, animationKey, skipEntrance, speed, fontSize, playClick }: SplitFlapCharProps) {
  const displayChar = CHARSET.includes(char) ? actualChar : " " // Use the original actualChar (e.g. lowercase) for final display
  const isSpace = char === " "
  ...
  const tileDelay = 0 // Removed index staggering so all letters appear at once
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(() => {
      intervalRef.current = setInterval(() => {
        ...
        // Use lowercase variant if our string is mostly lowercase during flipping to make it match parallax.ai
        const randChar = CHARSET[Math.floor(Math.random() * CHARSET.length)]
        setCurrentChar(actualChar.toLowerCase() === actualChar ? randChar.toLowerCase() : randChar)
  ...
  // Make sure not to cause infinite loops here with dependencies
  }, [displayChar, isSpace, tileDelay, animationKey, skipEntrance, index, speed, playClick, actualChar])
  ...
  return (
    ...
    >
      <div className="absolute inset-x-0 top-1/2 h-[1px] bg-white/20 pointer-events-none z-10" />

      <div className="absolute inset-x-0 top-0 bottom-1/2 flex items-end justify-center overflow-hidden rounded-t-[0.15em]">
        <span
      ...
      <div className="absolute inset-x-0 top-1/2 bottom-0 flex items-start justify-center overflow-hidden rounded-b-[0.15em]">
        <span
      ...
      <motion.div
        ...
        className="absolute inset-x-0 top-0 bottom-1/2 origin-bottom overflow-hidden rounded-t-[0.15em]"
        style={{
          backgroundColor: bgColor,
          transformStyle: "preserve-3d",
