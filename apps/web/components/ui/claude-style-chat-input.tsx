...
const legacyMockMessages = [
    "Analyzed pasted text...",
    "Analyzed image...",
    "Analyzed document...",
];

const isLegacyMockMessage = (value: string) => (
    legacyMockMessages.includes(value.trim()) ||
    /^Analyzed \d+ files\.\.\.$/.test(value.trim())
);
...
export const ClaudeChatInput: React.FC<ClaudeChatInputProps> = ({ onSendMessage }) => {
    ...
    useEffect(() => {
        if (isLegacyMockMessage(message)) {
            setMessage("");
        }
    }, [message]);
                        ...
                        <div className="max-h-96 w-full overflow-y-auto custom-scrollbar font-sans break-words transition-opacity duration-200 min-h-[2.5rem] pl-1">
                            <textarea
                                ...
                                onChange={(e) => {
                                    ...
                                    setMessage(isLegacyMockMessage(nextMessage) ? "" : nextMessage);
                                }}
                                onPaste={handlePaste}
